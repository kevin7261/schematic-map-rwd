...
| **XLSX**       | 0.18.5 | Excel 文件處理 | [SheetJS](https://sheetjs.com/)             | Excel 數據載入和處理                         |
...
│   │   └── 📁 xlsx/             # 📈 Excel 統計數據
│   │       ├── Dengue Daily_台南市區_高雄市區.xlsx  # 每日病例統計
│   │       └── 台南市區.xlsx                    # 台南市區統計數據
 ...
 *
 * 本模組負責：
 * - 通用檔案載入（GeoJSON、Excel）
 * - GeoJSON 與 Excel 的合併（以鍵值對應）
...
原始數據 → dataProcessor → 標準化數據 → spatialAnalysis → 分析結果 → 視覺化
    ↓           ↓              ↓               ↓            ↓          ↓
GeoJSON/Excel → 數據清理 → 屬性標準化 → 空間計算 → 統計指標 → 地圖/圖表
```
...
const preloadData = async () => {
  const promises = [
    fetch('/data/geojson/DengueKS2014.geojson'),
    fetch('/data/xlsx/台南市區.xlsx'),
  ];
