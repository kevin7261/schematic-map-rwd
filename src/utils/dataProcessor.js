...
/**
 * 計算空間滯後值圖例資料並分配顏色給 features (包含0值分級)
 */
export function calculateClassification_SpatialLag(geoJsonData) {
  try {
    console.log('🎨 計算空間滯後值圖例並分配顏色（包含0值，使用綠色系）...');

    const values = geoJsonData.features
      .map((f) => parseFloat(f.properties.spatial_lag || 0))
      .filter((v) => !isNaN(v));
    ...
    // 分配顏色給 features
    geoJsonData.features.forEach((feature) => {
      const spatialLag = parseFloat(feature.properties.spatial_lag || 0);
      const baseColor = colorScale(spatialLag);
      const fillColor_spatialLag = d3
        .color(baseColor)
        .copy({ opacity: COLOR_CONFIG.OPACITY })
        .toString();

      feature.properties.fillColor_spatialLag = fillColor_spatialLag;

      if (feature.properties.tableData) {
        feature.properties.tableData.spatial_lag = spatialLag;
        feature.properties.tableData.spatial_lag_color = fillColor_spatialLag;
      }
    });

    // 計算每個分類的數量
    const validFeatures = geoJsonData.features.filter((f) => {
      const value = parseFloat(f.properties.spatial_lag || 0);
      return !isNaN(value) && value > 0;
    });
    ...
    // 將每個有效數據點分配到正確的區間
    validFeatures.forEach((feature) => {
      const value = parseFloat(feature.properties.spatial_lag || 0);
      let classIndex = 0;
    ...
    // 生成帶數量的圖例
    const spatialLagFormat = (d) => Math.round(d * 1000) / 1000; // 保留三位小數
    const legendData_SpatialLag = colors.map((color, index) => {
      let label = '';
      let extent = [];
      let count = preClassifiedData[index].length;
      ...
      if (index === 0) {
        const upperBound = thresholds[0];
        extent = [minValue, upperBound];
        label = `${spatialLagFormat(minValue)} - ${spatialLagFormat(upperBound)} (${count})`;
      } else if (index === colors.length - 1) {
        const lowerBound = thresholds[thresholds.length - 1];
        extent = [lowerBound, maxValue];
        label = `${spatialLagFormat(lowerBound)} - ${spatialLagFormat(maxValue)} (${count})`;
      } else {
        const lowerBound = thresholds[index - 1];
        const upperBound = thresholds[index];
        extent = [lowerBound, upperBound];
        label = `${spatialLagFormat(lowerBound)} - ${spatialLagFormat(upperBound)} (${count})`;
      }
    ...
    // 在圖例項目中加入缺值統計信息（與人口密度圖例格式一致）
    legendData_SpatialLag.forEach((item) => {
    ...
    // 驗證總數
    const totalLegendCount = legendData_SpatialLag.reduce(
      (sum, item) => sum + (item.count || 0),
      0
    );
    console.log(`✅ 空間滯後值圖例和顏色分配完成（綠色系，natural breaks，含計數）`);
    console.log(
      `📊 Spatial Lag 統計驗證: 圖例總數=${totalLegendCount}, 要素總數=${totalFeatureCount}, 缺值=${zeroOrMissingCount}`
    );
    return { geoJsonData, legendData_SpatialLag };
  } catch (error) {
    console.error('❌ 空間滯後值圖例計算失敗:', error);
    throw error;
  }
}

/**
 * 計算 Join Counts 二元分類圖例資料並分配顏色給 features
 */
export function calculateClassification_JoinCounts(geoJsonData, binaryThreshold = null) {
  try {
    ...
    if (actualThreshold === null || actualThreshold === undefined) {
      ...
      if (counts.length > 0) {
        ...
        actualThreshold = 0; // 如果沒有有效數據，使用0作為默認值
      }
    }

    console.log(`🎨 計算 Join Counts 二元分類圖例並分配顏色（閾值: ${actualThreshold}）...`);

    const colors = [d3.schemeSet3[0], d3.schemeSet3[1]]; // 0值和1值顏色

    // 分配顏色給 features
    geoJsonData.features.forEach((feature) => {
      const count = parseFloat(feature.properties.count || 0);
      const binaryValue = count > actualThreshold ? 1 : 0;
      const baseColor = colors[binaryValue];
      const fillColor_joinCounts = d3
        .color(baseColor)
        .copy({ opacity: COLOR_CONFIG.OPACITY })
        .toString();

      feature.properties.fillColor_joinCounts = fillColor_joinCounts;
      feature.properties.binaryValue = binaryValue;

      if (feature.properties.tableData) {
        feature.properties.tableData.joinCounts_color = fillColor_joinCounts;
        feature.properties.tableData.binaryValue = binaryValue;
      }
    });

    // 計算每個分類的數量
    const count0 = geoJsonData.features.filter((f) => {
      const count = parseFloat(f.properties.count || 0);
      return count <= actualThreshold;
    }).length;

    const count1 = geoJsonData.features.filter((f) => {
      const count = parseFloat(f.properties.count || 0);
      return count > actualThreshold;
    }).length;

    // 計算缺值統計（對於 Join Counts，通常沒有缺值，因為是基於 count 欄位的二元分類）
    const totalFeatureCount = geoJsonData.features.length;
    const validCount = count0 + count1;
    const missingCount = totalFeatureCount - validCount;

    const legendData_JoinCounts = [
      {
        color: d3.color(colors[0]).copy({ opacity: COLOR_CONFIG.OPACITY }).toString(),
        label: `0 (≤ ${actualThreshold}) (${count0})`,
        extent: [0, actualThreshold],
        binaryValue: 0,
        count: count0,
        summary: {
          missingCount,
      ...
      {
        color: d3.color(colors[1]).copy({ opacity: COLOR_CONFIG.OPACITY }).toString(),
        label: `1 (> ${actualThreshold}) (${count1})`,
        extent: [actualThreshold + 1, Infinity],
        binaryValue: 1,
        count: count1,
        summary: {
          missingCount,
    ...
    // 驗證總數
    const totalJoinCountsLegendCount = legendData_JoinCounts.reduce(
      (sum, item) => sum + (item.count || 0),
      0
    );
    console.log('✅ Join Counts 二元分類圖例和顏色分配完成（含計數）');
    console.log(
      `📊 Join Counts 統計驗證: 圖例總數=${totalJoinCountsLegendCount}, 要素總數=${totalFeatureCount}, 缺值=${missingCount}`
    );
    return { geoJsonData, legendData_JoinCounts };
  } catch (error) {
    console.error('❌ Join Counts 圖例計算失敗:', error);
    throw error;
  }
}
